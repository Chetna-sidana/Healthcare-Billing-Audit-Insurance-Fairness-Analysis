...
1. **Inconsistent or erroneous charges** that go unnoticed without systematic review
2. **Payer-linked bias** — where billing depends on who's paying rather than the service delivered
...
1. Cleaned inconsistent name formatting and removed duplicate records
2. Calculated average and standard deviation of billing amount per medical condition
3. Computed a Z-Score per patient to flag deviation from the norm — then **switched to a percentile-based method (top/bottom 5%)** after discovering the billing values were uniformly distributed, which made Z-scores unreliable for this dataset
4. Cross-checked average billing by insurance provider to test for payer-linked bias
5. Built a Power BI dashboard with KPI cards, condition-wise and insurance-wise billing charts, and an outlier distribution view, with interactive slicers
